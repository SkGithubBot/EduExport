function Adder(a, b, digitsA, digitsB) {
    ...
    this.showDecimals = function(deciA, deciB) {
        ...
        this.showSideLabel("\\text{" + "Sørg for, at decimaltegnene står under hinanden." + "}");
    }
}
...
function Multiplier(a, b, digitsA, digitsB, deciA, deciB) {
    ...
    this.showDecimalsInProduct = function() {
        ...
        graph.label([x, y + 2],
            "\\text{" + "Det øverste tal har " + KhanUtil.plural(deciA, "digit") + "til højre for decimaltegnet." + "}", "right");
        graph.label([x, y + 1],
            "\\text{" + "Det nederste tal har" + KhanUtil.plural(deciB, "digit") + "til højre for decimaltegnet." + "}", "right");
        graph.label([x, y],
            "\\text{ " + "Produktet har" + deciA + " + " + deciB + " = " + (deciA + deciB)
             + " cifre til højre for decimaltegnet." + "}", "right");
        graph.style({
            fill: "#000"
        }, function() {
...
function Divider(divisor, dividend, deciDivisor, deciDividend) {
    ...
    this.addDecimalRemainder = function() {
        ...
        graph.label([digitsDividend.length, 1],
                "\\text{ " + "Sæt et decimaltegn ind, og et nul og fortsæt med at dividere." + "}", "right");
    };
    ...
    this.shiftDecimals = function() {
        ...
        if (deciDivisor !== 0) {
            graph.label([digitsDividend.length + 1 + (deciDiff > 0 ? deciDiff : 0), 1],
                "\\text{ " + "Flyt decimal" + deciDivisor + " til højre." + " }", "right");
            graph.style({
                fill: "#000"
            }, function() {
            ...
            graph.label([digitsDividend.length + 0.5, 1.2],
                "\\text{ " + "Sæt decimal ind i " + " }", "right");
            graph.label([digitsDividend.length + 0.5, 0.8],
                "\\text{ " + "svaret (kvotienten)." + " }", "right");
        }
