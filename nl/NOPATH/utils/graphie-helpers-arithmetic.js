...
function Multiplier(a, b, digitsA, digitsB, deciA, deciB) {
    ...
    this.showDecimalsInProduct = function() {
        ...
        graph.label([x, y + 2],
            "\\text{" + "Het bovenste getal heeft" + KhanUtil.plural(deciA, "digit") + "rechts van het decimaalteken staan." + "}", "right");
        graph.label([x, y + 1],
            "\\text{" + "The bottom number has" + KhanUtil.plural(deciB, "digit") + "rechts van het decimaalteken staan." + "}", "right");
        graph.label([x, y],
...
function Divider(divisor, dividend, deciDivisor, deciDividend) {
    ...
    this.addDecimalRemainder = function() {
        ...
        graph.label([digitsDividend.length, 1],
                "\\text{ " + "Schrijf een decimaalteken en een nul en ga door met delen." + "}", "right");
    };
    ...
    this.shiftDecimals = function() {
        ...
        if (deciDivisor !== 0) {
            ...
            graph.label([digitsDividend.length + 0.5, 1.2],
                "\\text{ " + "Breng het decimaalteken naar boven in het" + " }", "right");
            graph.label([digitsDividend.length + 0.5, 0.8],
                "\\text{ " + "antwoord (de quotient)." + " }", "right");
        }
