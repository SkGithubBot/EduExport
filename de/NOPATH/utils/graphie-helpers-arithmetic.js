function Adder(a, b, digitsA, digitsB) {
    ...
    this.showDecimals = function(deciA, deciB) {
        ...
        this.showSideLabel("\\text{" + "Sicherstellen, dass die Dezimalzeichen uebereinander aufgereiht sind." + "}");
    }
}
...
function Multiplier(a, b, digitsA, digitsB, deciA, deciB) {
    ...
    this.showDecimalsInProduct = function() {
        ...
        graph.label([x, y + 2],
            "\\text{" + "Die obere Zahl hat " + KhanUtil.plural(deciA, "digit") + "rechts der Dezimalstelle." + "}", "right");
        graph.label([x, y + 1],
            "\\text{" + "Die untere Zahl hat " + KhanUtil.plural(deciB, "digit") + "rechts der Dezimalstelle." + "}", "right");
        graph.label([x, y],
            "\\text{ " + "Das Produkt hat " + deciA + " + " + deciB + " = " + (deciA + deciB)
             + " Stellen rechts der Dezimalstelle." + "}", "right");
        graph.style({
            fill: "#000"
        }, function() {
...
function Divider(divisor, dividend, deciDivisor, deciDividend) {
    ...
    this.addDecimalRemainder = function() {
        ...
        graph.label([digitsDividend.length, 1],
                "\\text{ " + "Schreibe ein Dezimalzeichen und eine Null und dividiere weiter." + "}", "right");
    };
    ...
    this.shiftDecimals = function() {
        ...
        if (deciDivisor !== 0) {
            graph.label([digitsDividend.length + 1 + (deciDiff > 0 ? deciDiff : 0), 1],
                "\\text{ " + "Die Dezimalstelle " + deciDivisor + " nach rechts schieben." + " }", "right");
            graph.style({
                fill: "#000"
            }, function() {
            ...
            graph.label([digitsDividend.length + 0.5, 1.2],
                "\\text{ " + "Bringe das Dezimalzeichen hoch in die" + " }", "right");
            graph.label([digitsDividend.length + 0.5, 0.8],
                "\\text{ " + "Loesung (der Quotient)." + " }", "right");
        }
